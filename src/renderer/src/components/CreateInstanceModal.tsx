...
export const CreateInstanceModal: React.FC<CreateInstanceModalProps> = ({ onClose, onCreated }) => {
  ...
  const loadMcVersions = async () => {
    ...
    try {
      const v = await window.electronAPI.versions.getMinecraftVersions();
      setMcVersions(v.filter(x => x.type === 'release'));
      if (v.length > 0) {
        setSelectedGameVersion(v.find(x => x.type === 'release')?.id || v[0].id);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleImportZip = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsImporting(true);
    try {
      // @ts-ignore - path exists in Electron File object
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="p-5 flex-1 overflow-y-auto">
          <form id="create-instance-form" onSubmit={handleCreate} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1.5 uppercase tracking-wider">Версия игры</label>
              <select
